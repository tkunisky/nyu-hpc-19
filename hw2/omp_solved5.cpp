...
#pragma omp parallel shared(a, b, nthreads, locka, lockb) private(tid)
  ...
  #pragma omp sections nowait
    ...
    #pragma omp section
      ...
      // SOLUTION:
      // Set the locks with a finer granularity: around each for loop, lock
      // the variables involved and then unlock. This lets the loops setting
      // `a` and `b` execute in parallel, while the subsequent loops updating
      // `a` and `b` each in terms of the other are forced to execute serially.
